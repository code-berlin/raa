...
class Menu_item_dao extends CI_Model{
	...
	public function get_menu_items_by_menu_id_and_parent_id($menu_id, $parent_id) {
		...
		$qry = "SELECT `menuitem`.`id`, `menuitem`.`contentId`, `menuitem`.`jumpmark`, `child`.`menu_title`, `child`.`slug`, `child`.`parent_id`, `child`.`image`, `parent`.`slug` as parent_slug FROM `menuitem`
