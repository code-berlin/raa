...
class Toast_all extends CI_Controller
{
	...
	/**
	 * Generate an xml report of the tests
	 *
	 */
        public function xml(){
            ...
            foreach ($test_files as $file)
            {
                $test_urls[] = site_url($this->test_dir . $file . '/show_results_xml');
                $test_names[] = $file;
            }

            if($test_files && is_array($test_files) && count($test_files)>0){

                $output .= '<?xml version="1.0" encoding="UTF-8"?>';
                $output .= '<testsuites>';
                $output .= $this->_curl_get_for_xml($test_urls, $test_names);
                $output .= '</testsuites>';
	 ...
	 */
	function _curl_get_for_xml($urls, $test_names)
	{
		$output = '';
		...
		foreach ($urls as $url){
                    ...
                    //enables basic auth for this call
                    //uses same credentials as used to call this controller
                    ...
                    $data = array('test_name' => $test_names[$i]);
